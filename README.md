...
本项目为 Inner Garden：情绪日记与记忆花园。项目结合 AI 对话，构建一个沉浸式心灵情绪日记记录界面。用户在记录心情的过程中，可以获得 AI 的温柔回应与情绪引导，系统会自动将对话内容整理成日记，并转化为可保存、可回顾的记忆卡片，最终形成一个属于用户自己的情绪记忆花园。

传统日记的痛点是：用户不知道写什么、懒得坚持、写完没有反馈。本项目的价值是：用户只需要倾诉、输入文字、使用语音表达，或上传一张图片，AI 就可以帮助用户慢慢梳理当天的情绪与事件，最后自动生成一篇结构化日记，并转化成一张可收藏的记忆卡片。长期使用后，Memory Garden 会变成用户自己的情绪记忆花园。

本项目和其他日记项目最大的差异化是：

1、界面 AI 的治愈、情绪安抚和日记记录。
2、不要求日记分析和解决用户的心理状态。
3、重点在记录回忆，而不是进行心理诊断
